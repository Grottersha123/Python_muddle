...
# Invert image
        ...
        for i in range(size[0]):
                for j in range(size[1]):
                        pixel = pixels[i, j]
                        if rgb['r']: pixel = (255 - pixel[0], pixel[1], pixel[2])    # invert red
                        if rgb['g']: pixel = (pixel[0], 255 - pixel[1], pixel[2])    # invert green
                        if rgb['b']: pixel = (pixel[0], pixel[1], 255 - pixel[2])    # invert blue
                        pixels[i, j] = pixel
...
# Convert images with .jpg or .png formats
	...
	# lists of images
        ...
        if rgb['r'] or rgb['g'] or rgb['b']: 
                rgb['flag'] = 1     # need invert
        else:
                rgb['flag'] = 0     # don't need invert
...
parser.add_argument("directory", help = "Folder that contains pictures")
